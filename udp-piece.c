    ...
    while (temp_size > HEAD_SIZE) {
        ...
        if (temp_size >= (HEAD_SIZE + data_len)) {
            ...
            if (udp_piece->total_size == 0)        // 重置后第一次收到数据
            {
                ...
                // 获取总的分片数量
                udp_piece->total_pieces =
                        (piece_buf[HEAD_POS_TOTAL_PIECES] << 8) + (piece_buf[HEAD_POS_TOTAL_PIECES + 1]);
                udp_piece->recv_len = 0;
                udp_piece->recv_pieces = 0;
                if (udp_piece->recv_buf)
                    free(udp_piece->recv_buf);
            ...
            UDP_DEBUG(
                    "buf size: %d, piece_data_len: %d, p_index: %d, recv_pieces: %d, total_size: %d, total_pieces: %d\n",
                    temp_size, data_len, p_index, udp_piece->recv_pieces, udp_piece->total_size,
                    udp_piece->total_pieces);
            ...
            if (udp_piece->recv_pieces == udp_piece->total_pieces) {
                ...
                if (udp_piece->recv_len == udp_piece->total_size) {
                    ...
                    UDP_ERR("recv_len != total_size! recv_len: %d, total_size: %d", udp_piece->recv_len,
                            udp_piece->total_size);
                    get_all_pieces = -1;
                }
            }
        } else {
...
int udp_piece_merge_ex(udp_piece_t* udp_piece, void* buf, int size) {
    ...
    while (circular_buffer_size(udp_piece->circular_buffer) > HEAD_SIZE) {
        ...
        if (circular_buffer_size(udp_piece->circular_buffer) >= (HEAD_SIZE + data_len)) {
            ...
            UDP_DEBUG(
                    "buf size: %d, piece_data_len: %d, p_index: %d, recv_pieces: %d, total_size: %d, total_pieces: %d",
                    circular_buffer_size(udp_piece->circular_buffer), data_len, p_index, udp_piece->recv_pieces,
                    udp_piece->total_size, udp_piece->total_pieces);
            ...
            if ((tmp_total_pieces != udp_piece->total_pieces) || (tmp_total_size != udp_piece->total_size)) {
                UDP_DEBUG(
                        "Discard current frame, total_pieces:%d, cur_total_pieces:%d, recv_pieces:%d, total_size:%d, cur_total_size:%d\n",
                        udp_piece->total_pieces, tmp_total_pieces, udp_piece->recv_pieces, udp_piece->total_size,
                        tmp_total_size);
            ...
            if (udp_piece->recv_pieces == udp_piece->total_pieces) {
                udp_piece->total_pieces = 0;
                udp_piece->recv_pieces = 0;
                if (udp_piece->recv_len ==
                    udp_piece->total_size) /* current frame is received completely, call Process() */
                {
                    /* 组成了一帧数据 */
                    get_all_pieces = 1;
                } else {
